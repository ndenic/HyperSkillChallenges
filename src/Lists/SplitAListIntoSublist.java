 ...
 * @author nikolad
 */
public class SplitAListIntoSublist {

    /**
     * It splits the passed list into a sequence of sublists with a predefined
     * size
     */
    public static <T> List<List<T>> splitListIntoSubLists(List<T> list, int subListSize) {
        ...
        for (int i = 0; i < list.size(); i += subListSize) {
            sublists.add(list.subList(i, i + subListSize >= list.size() ? list.size() - 1 : i + subListSize));
        }
    ...
    public static void main(String[] args) {
        ...
        final List<List<Integer>> subLists = SplitAListIntoSublist.splitListIntoSubLists(list, subListSize);
